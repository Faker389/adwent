...
import { getQuestionStatus, getTimeRemaining, QuestionStatus } from '../lib/questions';
import { Question } from '../lib/userModel';
...
export const CalendarWindow = ({ day, questions, onClick, style, isAnswered = false }: CalendarWindowProps) => {
  ...
  useEffect(() => {
    const updateStatus = () => {
      ...
      if (question) {
        ...
        const targetDate = question.openDate instanceof Timestamp 
          ? question.openDate.toDate() 
          : new Date(question.openDate);
        ...
        if (diff > 0) {
          ...
          if (deadlineDiff > 0) {
            const dDays = Math.floor(
              deadlineDiff / (1000 * 60 * 60 * 24)
            );
            const dHours = Math.floor(
              (deadlineDiff % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
            );
            const dMinutes = Math.floor(
              (deadlineDiff % (1000 * 60 * 60)) / (1000 * 60)
            );
            ...
            if (dDays > 0) {
              ...
              setCurrentTime(
                `${dHours}:${dMinutes.toString().padStart(2, '0')}:${dSeconds
                  .toString()
                  .padStart(2, '0')}`
              );
            } else {
  ...
  const isClickable = status === 'available' || status === 'expiring';
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        isAnswered && "ring-2 ring-green-400 ring-offset-2 ring-offset-transparent",
        isSpecialDay
    ...
    >
      ...
      <div className={cn(
        ...
        isClickable && (isSpecialDay ? "bg-gradient-to-br from-yellow-400/30 to-amber-500/30" : "bg-gradient-to-br from-red-400/20 to-red-600/20")
      )} />
      
      {/* Status Icon */}
      {isLocked && <Lock className="w-3 h-3 sm:w-5 sm:h-5 text-gray-400 relative z-10" />}
      {isExpired && <Lock className="w-3 h-3 sm:w-5 sm:h-5 text-gray-500 relative z-10" />}
      {isAnswered && isClickable && <CheckCircle className="w-3 h-3 sm:w-5 sm:h-5 text-green-400 relative z-10" />}
      {!isAnswered && isClickable && !isLocked && (
        <Icon className={cn(
          "w-3 h-3 sm:w-5 sm:h-5 relative z-10",
          isSpecialDay ? "text-yellow-300" : status === 'expiring' ? "text-orange-300" : "text-red-300"
        )} />
      )}
      ...
      <span className={cn(
        ...
        isLocked || isExpired ? "text-gray-400" :
        isSpecialDay ? "text-yellow-300" : 
        status === 'expiring' ? "text-orange-300" :
        "text-white"
      )}>
        {day}
      </span>
      ...
      <div className={cn(
        ...
        {status === 'expiring' && <AlertTriangle className="w-2 h-2 sm:w-3 sm:h-3" />}
        {isLocked && <Clock className="w-2 h-2 sm:w-3 sm:h-3" />}
        {isClickable && !isExpired && <Clock className="w-2 h-2 sm:w-3 sm:h-3" />}
      ...
      {isClickable && (
        <>
          ...
          <div className="hidden sm:block absolute bottom-1 left-1 text-yellow-400/60 text-[8px] animate-pulse" style={{ animationDelay: '1s' }}>⭐</div>
        </>
      )}
    </motion.div>
  );
